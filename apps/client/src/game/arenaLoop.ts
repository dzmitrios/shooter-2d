...
import type { InputSendGate } from './inputSendGate.ts';
...
export function stepArena(ctx: ArenaStepContext): ArenaFrame {
  const { store, input, camera, senders, sendGate, now, dt, viewWidth, viewHeight } = ctx;
  ...
  if (store.spawned && !store.localIsDead && !inputBlocked) {
    ...
    if (input.isFiring() && sendGate.allowShoot(now)) {
